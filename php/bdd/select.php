...
include("conn.php");
$selection = new MaConnexion("Article", "", "root", "localhost");
...
if (!empty($select)) {
    ...
    if ($_POST['identifiant'] and $_POST['mdp'] = $select[0]["Mail"] and $select[0]["Mdp"]){
        session_unset();
        $_SESSION["idUser"]=$select[0]["Mail"] and
        ...
        ;
        // $moune = $select[0]["Mail"] and $select[0]["Mdp"];
        // var_dump($moune);
        // echo $moune;
